...
続いてフラグメントシェーダーを作成します。Resフォルダに`Terrain.frag`という名前のファイルを追加してください。こちらも基本的な部分は`StaticMesh.frag`と同じなので、「コピー＆貼り付け」で作ります。コメントにあるStaticMeshという部分をTerrainに変更するのを忘れないでください。
